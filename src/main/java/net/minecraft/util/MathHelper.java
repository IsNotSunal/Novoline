...
public class MathHelper {
    ...
    public static Vec3 getVec3(BlockPos pos, EnumFacing facing) {
        ...
        double random = ThreadLocalRandom.current().nextDouble(-0.25, 0.25), value = 0.5 + random;
    ...
    public static Vec3 _getVec3(BlockPos pos, EnumFacing face) {
        double x = pos.getX() + 0.5 + (double) face.getFrontOffsetX() / 2, //
                z = pos.getZ() + 0.5 + (double) face.getFrontOffsetZ() / 2, //
                y = pos.getY() + 0.5 + (double) face.getFrontOffsetY() / 2;

        if (face == EnumFacing.UP || face == EnumFacing.DOWN) {
            x += randomDouble(0.25D, -0.25D);
            z += randomDouble(0.25D, -0.25D);
        } else {
            y += randomDouble(0.25D, -0.25D);
        }
