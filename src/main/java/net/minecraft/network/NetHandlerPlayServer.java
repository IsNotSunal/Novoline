...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
    ...
    public void processPlayer(C03PacketPlayer packetIn) {
        ...
        if (this.func_183006_b(packetIn)) {
            ...
            if (!this.playerEntity.playerConqueredTheEnd) {
                ...
                double d3 = 0.0D;
                final double d4 = packetIn.getX() - this.lastPosX;
                final double d5 = packetIn.getY() - this.lastPosY;
                final double d6 = packetIn.getZ() - this.lastPosZ;
                ...
                if (this.hasMoved) {
                    ...
                    if (!this.serverController.isFlightAllowed() && !this.playerEntity.abilities.isAllowFlying() && !worldserver.checkBlockCollision(axisalignedbb)) {
                        if (d12 >= -0.03125D) {
    ...
    public void processPlayerBlockPlacement(C08PacketPlayerBlockPlacement packetIn) {
        ...
        if (packetIn.getPlacedBlockDirection() == 255) {
            ...
            if (this.hasMoved && this.playerEntity.getDistanceSq((double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.5D, (double) blockpos.getZ() + 0.5D) < 64.0D && !this.serverController.isBlockProtected(worldserver, blockpos, this.playerEntity) && worldserver.getWorldBorder().contains(blockpos)) {
                this.playerEntity.theItemInWorldManager.activateBlockOrUseItem(this.playerEntity, worldserver, itemstack, blockpos, enumfacing, packetIn.getFacingX(), packetIn.getFacingY(), packetIn.getFacingZ());
            }

            flag = true;
        } else {
            final ChatComponentTranslation chatcomponenttranslation = new ChatComponentTranslation("build.tooHigh", this.serverController.getBuildLimit());
    ...
    public void processConfirmTransaction(C0FPacketConfirmTransaction packetIn) {
        ...
        if (oshort != null && packetIn.getID() == oshort && this.playerEntity.openContainer.windowId == packetIn.getWindowId() && !this.playerEntity.openContainer.getCanCraft(this.playerEntity) && !this.playerEntity.isSpectator()) {
    ...
    public void processPlayerAbilities(C13PacketPlayerAbilities packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.playerEntity.getServerForPlayer());
        this.playerEntity.abilities.setFlying(packetIn.isFlying() && this.playerEntity.abilities.isAllowFlying());
    }
    ...
    public void processVanilla250Packet(C17PacketCustomPayload packetIn) {
        ...
        if ("MC|BEdit".equals(packetIn.getChannelName())) {
            ...
            } else if (this.playerEntity.canCommandSenderUseCommand(2, "") && this.playerEntity.abilities.isCreative()) {
