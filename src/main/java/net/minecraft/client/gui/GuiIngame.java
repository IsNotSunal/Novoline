...
import cc.novoline.events.events.ScoreBoardEvent;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.mc.gameSettings.thirdPersonView == 0 && itemstack != null && itemstack.getItem() == Item
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            if (k1 > 8) {
                ...
                this.getFontRenderer()
                        .drawString(this.recordPlaying, -this.getFontRenderer().getStringWidth(this.recordPlaying) / 2,
                                -4, i1 + (k1 << 24 & -16777216));
                GlStateManager.disableBlend();
        ...
        if (this.field_175195_w > 0) {
            ...
            if (this.field_175195_w <= this.field_175193_B) l1 = (int) (f4 * 255.0F / (float) this.field_175193_B);

            l1 = clamp_int(l1, 0, 255);
        ...
        if (scoreplayerteam != null && ViaFabric.clientSideVersion == 47) {
        ...
        if (!this.mc.gameSettings.keyBindPlayerList.isKeyDown() || this.mc
                .isIntegratedServerRunning() && this.mc.player.connection.getPlayerInfoMap()
        ...
        EventManager.call(new Render2DEvent(scaledresolution, partialTicks));
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        GlStateManager.disableLighting();
        GlStateManager.enableAlpha();
    }

    private void renderTooltip(ScaledResolution sr, float partialTicks) {
    ...
    protected boolean showCrosshair() {
        if (this.mc.gameSettings.showDebugInfo && !this.mc.player
                .hasReducedDebug() && !this.mc.gameSettings.reducedDebugInfo || Novoline.getInstance()
                .getModuleManager().getModule(Crosshair.class).isEnabled()) {
            return false;
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        List<Score> arrayList = collection.stream()
        ...
        int i = (int) getWidth(p_180475_1_.getDisplayName());

        for (Object score : arraylist1) {
            ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(((Score) score).getPlayerName());
            String s = ScorePlayerTeam.formatPlayerName(scoreplayerteam,
                    ((Score) score).getPlayerName()) + ": " + EnumChatFormatting.RED + ((Score) score).getScorePoints();
            i = (int) Math.max(i, getWidth(s));
        }

        int j1 = arraylist1.size() * getHeight();
        int k1 = p_180475_2_.getScaledHeight() / 2 + j1 / 3 + Novoline.getInstance().getModuleManager()
                .getModule(HUD.class).getScoreboard().get();
        byte b0 = 3;
        int j = p_180475_2_.getScaledWidth() - i - b0 + (isClientStyle() ? 6 : 9);
        int k = 0;
        ...
        for (Object score1 : arraylist1) {
            ...
            EventManager.call(new ScoreBoardEvent(k, s1));

            int l = k1 - k * getHeight();
            int i1 = p_180475_2_.getScaledWidth() - b0 + 3;
            int fix = isClientStyle() ? 1 : 0;

            if (Novoline.getInstance().getModuleManager().getModule(HUD.class).getHudElements()
                    .contains("Scoreboard")) {
                drawRect(j - 2, l - fix, i1, l + getHeight() - fix, 1342177280);

                boolean hide = Novoline.getInstance().getModuleManager().getModule(Streamer.class)
                        .isEnabled() && Novoline.getInstance().getModuleManager().getModule(Streamer.class)
                        .isHideServerId().get();

                if (hide && k == arraylist1.size()) {
                    List<String> splitted = new ArrayList<>(Arrays.asList(s1.split(" ")));

                    if (!splitted.isEmpty() && splitted.get(0).contains("/")) {
                        s1 = s1.replace(s1.substring(splitted.get(0).length()), "");
                    }
                }

                if (!s1.contains("novoline")) {
                    this.drawString(s1, j, l, 553648127);
                } else {
                    if (isClientStyle()) {
                        RenderUtils.drawHorizontal(getClientFont(), s1.replace("\u00a7e", ""), j, l);
                    } else {
                        RenderUtils.drawHorizontal(s1.replace("\u00a7e", ""), j, l);
                    }
                }
            }
            ...
            if (k == arraylist1.size()) {
                ...
                if (Novoline.getInstance().getModuleManager().getModule(HUD.class).getHudElements()
                        .contains("Scoreboard")) {
                    drawRect(j - 2, l - getHeight() - (isClientStyle() ? 2 : 1), i1, l - 1, 1610612736);
                    drawRect(j - 2, l - 1 + fix, i1, l, 1342177280);
                    drawString(s3, j + i / 2F - getWidth(s3) / 2F - 3F, l - getHeight(), 553648127);
                }
            }
        }
    }
    ...
    private void renderPlayerStats(ScaledResolution p_180477_1_) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            int i = ceiling_float_int(entityplayer.getHealth());
            ...
            int l1 = ceiling_float_int((f + f1) / 2.0F / 10.0F);
            ...
            if (entityplayer.isPotionActive(Potion.regeneration)) {
                l2 = this.updateCounter % ceiling_float_int(f + 5.0F);
            }
            ...
            for (int j5 = ceiling_float_int((f + f1) / 2.0F) - 1; j5 >= 0; --j5) {
                int k5 = 16;
                ...
                int k3 = ceiling_float_int((float) (j5 + 1) / 10.0F) - 1;
                int l3 = i1 + j5 % 10 * 8;
                int i4 = k1 - k3 * i2;
            ...
            if (entity == null) {
                ...
                for (int l5 = 0; l5 < 10; ++l5) {
                    ...
                    if (entityplayer.getFoodStats()
                            .getSaturationLevel() <= 0.0F && this.updateCounter % (k * 3 + 1) == 0) {
            ...
            if (entityplayer.isInsideOfMaterial(Material.water)) {
                ...
                int j8 = ceiling_double_int((double) (i6 - 2) * 10.0D / 300.0D);
                int k6 = ceiling_double_int((double) i6 * 10.0D / 300.0D) - j8;
    ...
    private void renderBossHealth() {
        if (!Novoline.getInstance().getModuleManager().getModule(HUD.class).getHudElements().contains("Bossbar")) {
            return;
        }
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
            ...
            this.getFontRenderer()
                    .drawStringWithShadow(s, (float) (i / 2 - this.getFontRenderer().getStringWidth(s) / 2),
                            (float) (b0 - 10), l);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            this.mc.getTextureManager().bindTexture(icons);
        }
    ...
    private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_) {
        if (!Config.isVignetteEnabled()) {
            ...
            p_180480_1_ = clamp_float(p_180480_1_, 0.0F, 1.0F);
            ...
            if (f > 0.0F) {
                ...
                GlStateManager
                        .color(this.prevVignetteBrightness, this.prevVignetteBrightness, this.prevVignetteBrightness,
                                1.0F);
            }
    ...
    public void updateTick() {
        ...
        if (this.mc.player != null) {
            ...
            } else if (this.highlightingItemStack != null && itemstack.getItem() == this.highlightingItemStack
                    .getItem() && ItemStack.areItemStackTagsEqual(itemstack, this.highlightingItemStack) && (itemstack
                    .isItemStackDamageable() || itemstack.getMetadata() == this.highlightingItemStack.getMetadata())) {
    ...
    private boolean isClientStyle() {
        return Novoline.getInstance().getModuleManager().getModule(HUD.class).isEnabled() &&
                Novoline.getInstance().getModuleManager().getModule(HUD.class).getScoreBoardStyle();
    }
    ...
    private int getHeight() {
        return isClientStyle() ? getClientFont().getHeight() + 2 : getVanillaFont().getHeight();
    }
