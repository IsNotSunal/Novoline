...
public class GuiNewChat extends Gui {
    ...
    public void drawChat(int updateCounter) {
        ...
        if (i1 != 0) {
            RenderUtils.drawBorderedRect(-1, v - (isClientStyle() ? 11 : 10),
    ...
    public void printChatMessage(IChatComponent p_146227_1_) {
        String text = p_146227_1_.getFormattedText();

        if (!p_146227_1_.getSiblings().isEmpty()) {
            for (IChatComponent component : p_146227_1_.getSiblings()) {
                String componentText = component.getFormattedText();

                for (NetworkPlayerInfo playerInfo : mc.getNetHandler().getPlayerInfoMap()) {
                    String name = playerInfo.getGameProfile().getName();
                    UserEntity user = Novoline.getInstance().getIRCUser(name);

                    if (componentText.contains(name) && name.length() > 1 && user != null) {
                        text = text.replace(name, name + " \u00A77(\u00A7b" + user.getUsername() + "\u00A77)\u00A7r");
                        IChatComponent finalComponent = new ChatComponentText(text).setChatStyle(component.getChatStyle());
                        this.printChatMessageWithOptionalDeletion(finalComponent, 0);
                        return;
                    }
                }
            }

        } else for (NetworkPlayerInfo playerInfo : mc.getNetHandler().getPlayerInfoMap()) {
            String name = playerInfo.getGameProfile().getName();
            UserEntity user = Novoline.getInstance().getIRCUser(name);

            if (user != null && name.length() > 1 && text.contains(name)) {
                text = text.replace(name, name + " \u00A77(\u00A7b" + user.getUsername() + "\u00A77)\u00A7r");
                this.printChatMessageWithOptionalDeletion(new ChatComponentText(text).setChatStyle(p_146227_1_.getChatStyle()), 0);
                return;
            }
        }
