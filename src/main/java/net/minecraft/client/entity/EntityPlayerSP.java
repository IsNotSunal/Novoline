    ...
    public void sendChatMessage(String message) {
        for (String command : this.coomands) {
            if (message.toLowerCase().startsWith(command)) {
                ...
                String commandFixed = msg[0].toLowerCase() + message.replace(msg[0], "");
                this.mc.getNovoline().getNovoCommandHandler().executeCommand(this, commandFixed);
                return;
            }
        }
    ...
    public void onLivingUpdate() {
        ...
        if (this.isUsingItem() && !this.isRiding()) {
            ...
            if (!event.isCancelled()) {
                this.movementInput().setMoveStrafe(this.movementInput().moveStrafe() * 0.2F);
                this.movementInput().setMoveForward(this.movementInput().moveForward() * 0.2F);
        ...
        if (this.onGround && !flag1 && !flag2 && this.movementInput().moveForward() >= f && !this
        ...
        if (!this.isSprinting() && this.movementInput().moveForward() >= f && flag3 && !this.isUsingItem() && !this
...
/*    public Block groundBlock() {
    ...
    public double constantFallDistance() {
        for (int i = (int) this.posY; i > 0; i--) {
            ...
            if (block.isSolidFullCube() || block instanceof BlockGlass) {
