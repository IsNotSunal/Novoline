...
public final class RotationUtil {
    ...
    public static float[] getAngles(double x, double y, double z) {
        ...
        final double diffX = x - player.posX,
                diffY = y - (player.posY + player.getEyeHeight()),
                diffZ = z - player.posZ, dist = MathHelper.sqrt_double(diffX * diffX + diffZ * diffZ); // @on
    ...
    public static float[] getAngles(Vec3 vector) {
        ...
        final double diffX = posX - player.posX, diffY = posY - (player.posY + player.getEyeHeight()),
                diffZ = posZ - player.posZ, dist = MathHelper.sqrt_double(diffX * diffX + diffZ * diffZ); // @on

        final float yaw = (float) (Math.atan2(diffZ, diffX) * 180.0D / Math.PI) - 90.0F,
                pitch = (float) -(Math.atan2(diffY, dist) * 180.0D / Math.PI);

        return new float[]{MathHelper.wrapAngleTo180_float(player.rotationYaw + yaw - player.rotationYaw),
