...
public final class KillAura extends AbstractModule {
    ...
    /* properties @off */
    @Property("mode")
    private final StringProperty mode = PropertyFactory.createString("Switch").acceptableValues("Switch", "Single");
    ...
    public KillAura(@NonNull ModuleManager moduleManager) {
        ...
        Manager.put(new Setting("AURA_SORT", "Sort by", SettingType.COMBOBOX, this, sort, () -> mode().equalsIgnoreCase("Single")));
    ...
    public boolean shouldBlock() {
        return autoBlock.get() && target != null && heldSword() && target.isEntityAlive() && isEnabled() && !mc.playerController.isBreakingBlock() &&
                mc.player.getDistanceToEntity(target) <= (mc.player.canEntityBeSeen(target) ? blockRange() : wallRange()) && !isEnabled(Scaffold.class) && !longJumpIsWaiting();
    }
    ...
    private void attackEntity(Entity target) {
        if (timerAttack.delay(1000L / aps.get().longValue())) {
            ...
            if (ServerUtils.isHypixel() && shouldBlock()) {
    ...
    public void onSend(PacketEvent event) {
        if (event.getState().equals(PacketEvent.State.OUTGOING)) {
            ...
            if (shouldBlock() && event.getPacket() instanceof S08PacketPlayerPosLook) {
                sendUnBlock();
            }
        }
    }
    ...
    private boolean longJumpIsWaiting() {
        return isEnabled(LongJump.class) && getModule(LongJump.class).isWait();
    }
