    ...
    public void onUpdate(PlayerUpdateEvent event) {
        if (ServerUtils.isHypixel()) {
            for (NetworkPlayerInfo playerInfo : mc.getNetHandler().getPlayerInfoMap()) {
                ...
                if (timer.delay(1000) && !reported.contains(uuid) && !uuid.equals(mc.player.getEntityUniqueID())
                        && (novoline.getIRC() == null || novoline.getIRC().getUserManager().findByNickname(name) == null)) {
