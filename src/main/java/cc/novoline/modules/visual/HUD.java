...
public final class HUD extends AbstractModule {
    ...
    private final ListProperty<String> hudElements = createList("ModuleList", "UserInfo", "ArmorHUD", "Scoreboard", "Bossbar", "PotionHUD", "Coords", "Name", "Time", "FPS")
            .acceptableValues("ModuleList", "UserInfo", "TargetsList", "ArmorHUD", "Scoreboard", "Bossbar", "PotionHUD", "Coords", "Speed", "Name", "Time", "FPS", "Inventory");
    @Property("hud-font")
    ...
    private void drawArmorHUD(Render2DEvent event) {
        ...
        for (ItemStack item : stuff) {
            ...
            final int scaledHeight = resolution.getScaledHeightStatic(mc);
    ...
    private void drawUserInfo(Render2DEvent render2DEvent) {
        ...
        for (int i = 1; i <= 4 - Protection.serviceUid.length(); i++) {
            stringBuilder.append("0");
        }

        String uid = stringBuilder + Protection.serviceUid;
        String build = "Build - " + RESET + novoline.getVersion();
        final String info = GRAY + build + GRAY + " | " + "UID - " + WHITE + uid;

        if (hudFont.equalsIgnoreCase("Client")) {
            font.drawString(info, render2DEvent.getResolution().getScaledWidthStatic(mc) * getScale() - font.stringWidth(info) - 2,
                    render2DEvent.getResolution().getScaledHeightStatic(mc) * getScale() - font.getHeight() - 2, 0xFFFFFF, true);
        } else {
    ...
    private void drawPotionHUD(Render2DEvent event) {
        ...
        for (PotionEffect effect : mc.player.getActivePotionEffects()) {
            ...
            final int yChat = mc.ingameGUI.getChatGUI().getChatOpen() && !hudElements.contains("UserInfo") ? 5 : -10;
            ...
            if (hudFont.equalsIgnoreCase("Client")) {
                ...
                mc.fontRendererObj.drawStringWithShadow(pType, scaledWidth - mc.fontRendererObj.getStringWidth(pType) - 1,
                        scaledHeight - mc.fontRendererObj.getHeight() + y - (hudElements.contains("UserInfo") ? 23.5F : 13F) - yChat, rgbColor);
            }
    ...
    private void drawFPS(@NonNull Render2DEvent event) {
        ...
        final String fpsFont = RESET + "FPS: " + WHITE + mc.getDebugFPS(), fpsVanilla = RESET + "FPS: " + WHITE + mc.getDebugFPS();
    ...
    private void drawSpeed(@NonNull Render2DEvent event) {
        ...
        double value = mc.player.getLastTickDistance() * 20;

        if (isEnabled(Scaffold.class) && getModule(Scaffold.class).getBlockCount() > 0 && getModule(Scaffold.class).spoofValue()) {
            value = mc.player.getBaseMoveSpeed(mc.player.getBySprinting(true), 0.2) * (mc.player.isPotionActive(Potion.moveSpeed) ? 18.5 : 19.8);
        } else if (isEnabled(Flight.class) && getModule(Flight.class).spoofValue()) {
            value = mc.player.getBaseMoveSpeed() * 1.8;
        }

        String speed = String.format("%.2f", value * (mc.timer.timerSpeed < 1 ? 1 : mc.timer.timerSpeed));
